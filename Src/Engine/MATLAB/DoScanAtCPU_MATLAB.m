...
try
    ...
    while (i < MaxStep-1)
        ...
        dt = utsLine(min(utsi+2, MaxutsStep))-utsLine(utsi+1);
        VVar.dt = dt;
        utsi = min(utsi+1, MaxutsStep);
        VVar.utsi = utsi;
            ...
            for j=0:(flag(1)-1)
                ...
                while (s < MaxrfStep)
                    if (rfCoil == rfCoilLine(s+1))
                        ...
                        VVar.rfAmp(TxCoili) = rfAmp(TxCoili);
                        VVar.rfFreq(TxCoili) = rfFreq(TxCoili);
                        VVar.rfPhase(TxCoili) = rfPhase(TxCoili);
                        break;
                    end
                    s = s + 1;
                end
        ...
        elseif (dt < 0)% /* uncontinuous time point process */
            ...
            switch (RunMode)
                case 1 %/* spin rotation simulation & rf simulation */
                    disp("RunMode 1: NOT IMPLEMENTED!");
                    %MzsBase = MzBase;
                    %MysBase = MyBase;
                    %MxsBase = MxBase;
            ...
            for Spini = 0:(SpinNum-1)
                ...
                if (ADC == 1)
                    ...
                    for RxCoili = 0:(RxCoilNum-1)%  /* signal acquisition per Rx coil */
                        %/* RxCoil sensitivity */
                        if (RxCoilDefault == 0)
                            disp("RxCoilDefault==0: NOT IMPLEMENTED");
                ...
                for Slicei=0:(SpinMxSliceNum-1)
                    ...
                    VObj.Mx(offset:(offset+SpinMxNum-1)) = Mx;
                    VObj.My(offset:(offset+SpinMxNum-1)) = My;
                    VObj.Mz(offset:(offset+SpinMxNum-1)) = Mz;
                    if (false && Slicei==0)
